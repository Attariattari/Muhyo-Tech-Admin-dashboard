...
export default function EditorialPlannerPage() {
  ...
  return <main className="mx-auto max-w-[1500px] space-y-6 pb-12"><section className="relative overflow-hidden rounded-[2rem] border border-border/70 bg-card p-6 shadow-sm md:p-8"><div className="pointer-events-none absolute -right-20 -top-24 h-72 w-72 rounded-full bg-accent/15 blur-3xl" /><div className="relative flex flex-col justify-between gap-6 lg:flex-row lg:items-center"><div className="flex items-start gap-4"><Link href="/admin/blogs" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-border bg-background text-muted-foreground hover:text-accent"><ArrowLeft className="h-4 w-4" /></Link><div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.2em] text-accent"><Sparkles className="h-3.5 w-3.5" /> Professional editorial intelligence</div><h1 className="mt-2 text-3xl font-black tracking-[-.045em] text-foreground md:text-4xl">Editorial planner</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Generates protected Core Pillar clusters, standalone authority articles and only source-verified technology trends in one professional catalog.</p></div></div><div className="flex flex-wrap gap-2"><Link href="/admin/blog-topics/new" className="inline-flex h-11 items-center gap-2 rounded-xl border border-border bg-background px-4 text-xs font-bold text-foreground hover:border-accent/30"><Plus className="h-4 w-4" /> Add manual topic</Link><button onClick={refill} disabled={refilling} className="inline-flex h-11 items-center gap-2 rounded-xl bg-accent px-5 text-xs font-bold text-accent-foreground shadow-lg shadow-accent/20 disabled:opacity-50">{refilling ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />} Generate professional catalog</button></div></div></section>
    ...
    {generation.open && mounted && typeof document !== "undefined" && createPortal(
      <div
        className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/75 dark:bg-black/85 p-4 sm:p-6 backdrop-blur-xl animate-in fade-in duration-200"
        role="dialog"
        aria-modal="true"
        aria-label="AI topic generation progress"
      >
        <div className="relative w-full max-w-lg overflow-hidden rounded-[2rem] border border-border/80 bg-card shadow-2xl backdrop-blur-2xl ring-1 ring-white/10">
          ...
          <div className="border-b border-border/70 p-6 sm:p-7 bg-gradient-to-b from-accent/5 to-transparent">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[9px] font-black uppercase tracking-[.25em] text-accent">AI editorial engine</p>
                  <h2 className="mt-1 text-xl sm:text-2xl font-black tracking-tight text-foreground">
                    {generation.status === "success"
                      ? "Topics are ready"
                      : generation.status === "error"
                        ? "Generation needs attention"
                        : "Generating unique topics"}
                  </h2>
                </div>
              </div>
              {generation.status === "error" && (
            ...
            <p className="mt-3 text-xs sm:text-sm leading-relaxed text-muted-foreground">{generation.message}</p>
          </div>
          <div className="space-y-2.5 p-6 sm:p-7">
            {generationSteps.map(({ label, detail, Icon }, index) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3.5 rounded-2xl border p-3.5 transition-all duration-300 ${
                    complete
                      ...
                      : active
                        ? "border-accent/40 bg-accent/10 shadow-lg shadow-accent/5 ring-1 ring-accent/20"
                ...
                >
                  <span
                    className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border transition ${
                      complete
                        ...
                        : active
                          ? "border-accent/40 bg-accent text-accent-foreground shadow-md shadow-accent/20"
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={`text-xs font-bold ${active ? "text-foreground font-black" : "text-foreground"}`}>{label}</p>
                    <p className={`mt-0.5 text-[10px] ${active ? "text-accent font-semibold" : "text-muted-foreground"}`}>{detail}</p>
                  </div>
                </div>
              );
            })}
          ...
          <div className="flex items-center justify-between border-t border-border/70 bg-muted/30 px-6 py-4">
            ...
            {generation.status === "success" && (
              <span className="text-[10px] font-bold text-status-success">
                ✓ {generation.added} topics processed · closing automatically
              </span>
            )}
            {generation.status === "error" && (
              <button
                onClick={refill}
                className="inline-flex h-9 items-center gap-2 rounded-xl bg-accent px-4 text-[10px] font-black uppercase tracking-wider text-accent-foreground shadow-md transition hover:brightness-110"
              >
          ...
          {generation.error && (
            <div className="mx-6 mb-6 rounded-xl border border-status-danger/30 bg-status-danger/10 p-3.5 text-xs leading-5 text-status-danger">
              <strong className="block text-[9px] font-black uppercase tracking-wider text-status-danger">Error details</strong>
              {generation.error}
            </div>
          )}
        </div>
    ...
    {inspectedTopic && mounted && typeof document !== "undefined" && createPortal(
      <div
        className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/75 dark:bg-black/85 p-4 sm:p-6 backdrop-blur-xl animate-in fade-in duration-200"
        role="dialog"
        aria-modal="true"
      >
        <div className="relative w-full max-w-2xl overflow-hidden rounded-[2rem] border border-border/80 bg-card shadow-2xl backdrop-blur-2xl ring-1 ring-white/10">
          <div className="flex items-center justify-between border-b border-border/70 p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[9px] font-black uppercase tracking-[.2em] text-accent">Topic Intelligence Inspection</p>
                <h2 className="text-lg font-black text-foreground">{inspectedTopic.title}</h2>
              </div>
            </div>
            <button
          ...
          <div className="max-h-[70vh] overflow-y-auto p-6 space-y-6">
            <div>
              <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-3">5-Dimension Score Breakdown</h4>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                <div className="rounded-xl border border-border bg-background p-3">
                  <p className="text-[10px] font-semibold text-muted-foreground">Demand Score</p>
                  <p className="text-lg font-black text-accent">{inspectedTopic.intelligence?.scoreBreakdown?.demandScore || 80}/100</p>
                </div>
                <div className="rounded-xl border border-border bg-background p-3">
                  <p className="text-[10px] font-semibold text-muted-foreground">Intent Score</p>
                  <p className="text-lg font-black text-accent">{inspectedTopic.intelligence?.scoreBreakdown?.intentScore || 75}/100</p>
                </div>
                <div className="rounded-xl border border-border bg-background p-3">
                  <p className="text-[10px] font-semibold text-muted-foreground">Authority Score</p>
                  <p className="text-lg font-black text-accent">{inspectedTopic.intelligence?.scoreBreakdown?.authorityScore || 85}/100</p>
                </div>
                <div className="rounded-xl border border-border bg-background p-3">
                  <p className="text-[10px] font-semibold text-muted-foreground">Commercial Score</p>
                  <p className="text-lg font-black text-accent">{inspectedTopic.intelligence?.scoreBreakdown?.commercialScore || 70}/100</p>
                </div>
                <div className="rounded-xl border border-border bg-background p-3">
                  <p className="text-[10px] font-semibold text-muted-foreground">Service Relevance</p>
                  <p className="text-lg font-black text-accent">{inspectedTopic.intelligence?.scoreBreakdown?.serviceRelevanceScore || 80}/100</p>
                </div>
                <div className="rounded-xl border border-border bg-card p-3">
                  <p className="text-[10px] font-semibold text-accent">Overall Score</p>
                  <p className="text-lg font-black text-foreground">{inspectedTopic.intelligence?.opportunityScore || inspectedTopic.priority || 75}/100</p>
                </div>
              </div>
            </div>
            <div>
              <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Service Mapping & Recommendation</h4>
              <div className="rounded-xl border border-border bg-background p-4 space-y-2">
                <p className="text-xs text-foreground">
                  <strong className="text-accent">Primary Service:</strong> {inspectedTopic.intelligence?.primaryService ? `${inspectedTopic.intelligence.primaryService.title} (${inspectedTopic.intelligence.primaryService.relevanceScore}% Match)` : "No direct service mapping"}
                </p>
                <p className="text-xs text-foreground">
                  <strong className="text-accent">AI Recommendation:</strong> <span className="rounded bg-emerald-500/10 px-2 py-0.5 font-bold text-emerald-400">{inspectedTopic.intelligence?.aiRecommendation || "GENERATE"}</span>
                </p>
                <p className="text-xs text-muted-foreground">
                  <strong className="text-foreground">Focus Keyword:</strong> {inspectedTopic.focusKeyword || "N/A"}
                </p>
                <p className="text-xs text-muted-foreground">
                  <strong className="text-foreground">Problem Solved:</strong> {inspectedTopic.problem || "N/A"}
                </p>
              </div>
            </div>
          </div>
          <div className="flex justify-end border-t border-border/70 bg-muted/25 px-6 py-4">
            <button onClick={() => setInspectedTopic(null)} className="rounded-xl bg-accent px-5 py-2 text-xs font-bold text-accent-foreground transition hover:brightness-110">
              Close Inspection
            </button>
          </div>
        </div>
